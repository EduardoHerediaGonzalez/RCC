...
void RCC_vAPB2PeripheralReset(uint8 u8Peripheral)
{
	ASSERT((uint8)(u8Peripheral < (uint8)RCC_enTotalOfAPB2Peripherals), (sint8)ASSERT_nVALUE_OUT_OF_RANGE);

	RCC_s_pstRegisters->APB2RSTR |= (uint32)(1<<u8Peripheral);
}
